...
export function TypingTestScreen({ username, onTestComplete }: TypingTestScreenProps) {
  ...
  const calculateMetrics = useCallback(() => {
    if (startTimeRef.current) {
      ...
      if (elapsedMinutes > 0) {
        const calculatedWPM = Math.round(totalWordsTypedRef.current / elapsedMinutes);
        setWordsPerMin(calculatedWPM);
      }
    }

    if (totalWordsTypedRef.current > 0) {
      const incorrectWords = totalWordsTypedRef.current - correctWordsRef.current;
      const calculatedAccuracy = Math.max(0, Math.round(((totalWordsTypedRef.current - incorrectWords) / totalWordsTypedRef.current) * 100));
      setAccuracy(calculatedAccuracy);
    } else {
      setAccuracy(100);
    }
  ...
  const endTest = useCallback(() => {
    ...
    const finalWPM = elapsedMinutes > 0 ? Math.round(totalWordsTypedRef.current / elapsedMinutes) : 0;
    const incorrectWords = totalWordsTypedRef.current - correctWordsRef.current;
    const finalAccuracy = totalWordsTypedRef.current > 0 
      ? Math.max(0, Math.round(((totalWordsTypedRef.current - incorrectWords) / totalWordsTypedRef.current) * 100))
      : 100;
  ...
  const handleInputChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    totalWordsTypedRef.current = typedWords.length;
